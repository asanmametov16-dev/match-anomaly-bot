...
детекторам, **не-направленный** (как spread): в `side`/`agreement` не
участвует. **Входит в `ALERT_DETECTORS`** — он на детерминированном
тождестве, а не статистике, ждать «созревания» незачем; вклад только в
`score`/число детекторов precision-gate, не в направление. No-op без
форы 0.0 — лучше молчать, чем шуметь. (`exotic_spread` остаётся вне
`ALERT_DETECTORS` — он шумный.)
...
`compute_score` ≥ `signal_score_threshold`, направление подтверждено ≥
`signal_min_directional` **разными** направленными детекторами и
согласие ≥ `signal_min_agreement`. **CLV влияет ТОЛЬКО через
`compute_score`** (× `detector_multiplier`). Детектор, чей множитель
**на полу клампа** (`clv_calibration_min_multiplier` — исторически
доказанный шум), исключается из решения — как `model_gap` из лиги
`unreliable` (`_exclude`, meta `dropped_noisy_clv`). Это P2-замена
прежнего `mean_mult ≥ signal_min_clv_multiplier`: невзвешенное среднее
дважды учитывало CLV, тянулось ненаправленными детекторами и **щёлкало
в «нет сигналов» ровно при созревании CLV на строгом профиле** (порог
1.0 стоял на cold-start-точке). `signal_min_clv_multiplier` —
DEPRECATED, больше не гейт (оставлен для совместимости .env). Голосование **по детекторам, не по
хитам**: каждый направленный детектор даёт один голос (мажоритарная
сторона своих хитов) — иначе один детектор с несколькими хитами создаёт
мнимый «100% консенсус». `compute_score` тоже считает вес **по разным
детекторам**: spread, сработавший на home+draw+away, не утраивает счёт. Пайплайн **сохраняет всё** и штампует каждой
...
├── test_result_checker_dedup.py     # дедуп MatchResult (dual-source)
├── test_maintenance.py              # выборочный сброс detector-данных
